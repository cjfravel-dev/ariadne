...
### Index Repartitioning for Large Joins

When joining against very large indexes, the `explode` operation on index array columns can produce massive intermediate DataFrames that cause `FetchFailedException` during shuffles. The optional `indexRepartitionCount` configuration allows repartitioning the index DataFrame before these expensive operations to spread data across more partitions.

**Where repartitioning is applied:**

1. **Before explode in `locateFilesFromDataFrame`**: The index DataFrame is repartitioned before array columns are exploded during file location, reducing per-partition memory pressure
2. **After `readFiles` in `joinDf`**: The DataFrame loaded from matched files is repartitioned before the subsequent join operations

**Configuration:**

- `spark.ariadne.indexRepartitionCount`: Number of partitions to repartition the index DataFrame to during joins (default: not set — no repartitioning)

**When to use:**

- Set this when experiencing `FetchFailedException` during joins to large indexes
- Typical values range from 100-500 depending on cluster size and data volume
- Not needed for small/medium indexes as repartitioning adds shuffle overhead
