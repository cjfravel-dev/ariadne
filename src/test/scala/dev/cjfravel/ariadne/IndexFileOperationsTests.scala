...
class IndexFileOperationsTests extends SparkTests with Matchers {
  ...
  test("should apply computed indexes correctly") {
    val index =
      Index("computed_index_test", csvSchema, "csv", Map("header" -> "true"))
  ...
  test("should apply exploded field transformations correctly") {
    ...
    val readOptions = Map("multiLine" -> "true")
    val index =
      Index("exploded_field_test", explodedSchema, "json", readOptions)
  ...
  test("should handle different file formats") {
    // Test CSV
    val csvIndex =
      Index("format_csv_test", csvSchema, "csv", Map("header" -> "true"))
    csvIndex.format should be("csv")

    // Test JSON
    val jsonIndex =
      Index("format_json_test", jsonSchema, "json", Map("multiLine" -> "true"))
  ...
  test("should combine computed indexes and exploded fields") {
    ...
    // Write test data
    val tempPath =
      s"${System.getProperty("java.io.tmpdir")}/combined_test_${System.currentTimeMillis()}"
    ...
    try {
      ...
      val priorityFiles =
        index.locateFiles(Map("high_priority" -> Array("high")))
