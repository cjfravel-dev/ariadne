...
class IndexQueryOperationsTests extends SparkTests with Matchers {
  ...
  test("should locate files by single index value") {
    val index =
      Index("query_single_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should locate files by multiple index values") {
    val index =
      Index("query_multiple_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should return empty set for non-existent values") {
    val index =
      Index("query_empty_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should locate files using computed indexes") {
    val index =
      Index("query_computed_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should generate statistics for indexed columns") {
    val index =
      Index("query_stats_test", testSchema, "csv", Map("header" -> "true"))
    ...
    val fileCount =
      stats.select("FileCount").collect()(0).getAs[Long]("FileCount")
  ...
  test("should handle printIndex without errors") {
    val index =
      Index("query_print_test", testSchema, "csv", Map("header" -> "true"))
