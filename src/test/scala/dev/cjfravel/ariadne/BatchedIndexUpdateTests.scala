...
class BatchedIndexUpdateTests extends SparkTests with Matchers {
  ...
  test("should create multiple batches based on distinct value limits") {
    ...
    try {
      val index =
        Index("multiple_batch_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should handle single file efficiently") {
    ...
    try {
      val index =
        Index("single_file_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should handle mixed file sizes with optimal batching") {
    ...
    try {
      val index =
        Index("mixed_size_test", testSchema, "csv", Map("header" -> "true"))
      ...
      val smallFileResults =
        index.locateFiles(Map("Id" -> Array(50))) // Should find small files
      val mediumFileResults =
        index.locateFiles(Map("Id" -> Array(250))) // Should find medium files
      val largeFileResults =
        index.locateFiles(Map("Id" -> Array(800))) // Should find large files
