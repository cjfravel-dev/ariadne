...
class IndexBuildOperationsTests extends SparkTests with Matchers {
  ...
  test("should build regular indexes correctly") {
    val index =
      Index("build_regular_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should build computed indexes correctly") {
    val index =
      Index("build_computed_test", testSchema, "csv", Map("header" -> "true"))
    ...
    val doubledFiles =
      index.locateFiles(Map("id_doubled" -> Array(2, 4, 6))) // 2*1, 2*2, 2*3
  ...
  test("should build exploded field indexes correctly") {
    ...
    // Write test data
    val tempPath =
      s"${System.getProperty("java.io.tmpdir")}/exploded_build_test_${System.currentTimeMillis()}"
    testData.write.mode("overwrite").json(tempPath)

    try {
      val readOptions = Map("multiLine" -> "true")
      val index =
        Index("build_exploded_test", explodedSchema, "json", readOptions)
  ...
  test("should handle mixed index types correctly") {
    ...
    try {
      ...
      val eventFiles =
        index.locateFiles(Map("event_id" -> Array("evt1", "evt2")))
      eventFiles should not be empty

      val priorityFiles =
        index.locateFiles(Map("priority_level" -> Array("high")))
  ...
  test("should handle update with no unindexed files") {
    val index =
      Index("no_unindexed_test", testSchema, "csv", Map("header" -> "true"))
  ...
  test("should handle incremental updates correctly") {
    val index =
      Index("incremental_test", testSchema, "csv", Map("header" -> "true"))
